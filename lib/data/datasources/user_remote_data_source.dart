...
import 'package:sell_streams/data/models/user_response_model.dart';
...
class UserRemoteDataSourceImpl implements UserRemoteDataSource {
  ...
  @override
  Future<UserModel> editCurrentUser(dynamic body) async {
    var user;
    String userId = await storage.read(key: APIConstants.USER_ID_KEY);
    if(userId != null){
      final response = await client.editProfileBio(pathSegment: 'edit-profile', body: body);
      user = UserResponseModel.fromJSON(response).user;
      print('user in urds is \t $user');
    }
    return user ?? null;
